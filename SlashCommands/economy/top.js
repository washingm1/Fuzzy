...
const { MessageActionRow, MessageButton, ContextMenuInteraction } = require('discord.js');
const Canvas = require('canvas');


module.exports = {
    name: "top",
    description: "View top users in the server",
...
console.log(interaction)
    ...
    let level = db.all().filter(data => data.ID.startsWith(`xptotal_646074330249429012_`)).sort((a, b) => b.data - a.data);
      ...
      console.log(memberFetch)
        var UserLevel = db.fetch(`guild_${guild.id}_level_${user.id}`)
      
        var xpNeeded = UserLevel * 850 + 500;
      
        var xp = db.get(`guild_${guild.id}_xp_${user.id}`)
      
       userBal = db.fetch(`money_646074330249429012_${user.id}`)
      
       xpUser = `${db.get(`guild_${guild.id}_xp_${user.id}`)} / ${xpNeeded}`
